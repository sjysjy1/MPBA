...
{'model':'WongLinf','attack':'MPBA_ada_step','p_norm':'L2','popsize':512,'init_pop_range':0.1,'generation_num':8000,'stepsize_start':0.1, 'batch_size':4,'mu_start':0.01,'mu_end':1.,'lower':0.1,'upper':1.,'rate_mutation':0.1,'rate_crossover':0.,'c':1},
{'model':'WongLinf','attack':'MPBA_ada_step','p_norm':'L2','popsize':512,'init_pop_range':0.1,'generation_num':10000,'stepsize_start':0.1, 'batch_size':4,'mu_start':0.01,'mu_end':1.,'lower':0.1,'upper':1.,'rate_mutation':0.1,'rate_crossover':0.,'c':1},
{'model':'WongLinf','attack':'SquareAttack','p_norm':'L2','eps':4,'steps':40000,'batch_size':batch_size},
{'model':'WongLinf','attack':'SquareAttack','p_norm':'L2','eps':5,'steps':40000,'batch_size':batch_size},
    ]
...
for item in list_para:
    ...
    if item['attack'] == 'MPBA':
        ...
        with torch.no_grad():
            for i in range(0,4,item['batch_size']):
                print('***************{}th batch***********'.format(int(i/item['batch_size'])))
                images=test_data_normalized[correct_index[i:i+item['batch_size']]].clone().to(device)
                labels=test_labels[correct_index[i:i+item['batch_size']]]
                ...
                _, labels_predict = torch.max(outs, 1)
        #        fig = plt.figure(figsize=(5, 15))
        #        plt.imshow(np.transpose(torchvision.utils.make_grid(adv_images[0].cpu().data, normalize=True), (1, 2, 0)))
        #        plt.title('predict{}'.format(labels_predict[0]))
        #        del adv_images
                torch.cuda.empty_cache()
                success = (labels_predict != labels)
                list_success_fail=list_success_fail+success.tolist()
        ...
        with torch.no_grad():
             for i in range(0,len(correct_index),item['batch_size']):
                ...
                list_success_fail = list_success_fail + success.tolist()
                if item['p_norm'] == 'L2':
                   perturbation = torch.norm((adv_images.cpu() - images).view(len(images), -1), p=2, dim=1)
                elif item['p_norm'] == 'Linf':
                   perturbation = torch.norm((adv_images.cpu()  - images).view(len(images), -1), p=float('inf'), dim=1)
                list_pert = list_pert + perturbation.tolist()
                print('perturbation is: ', torch.t(perturbation))
                print('average of perturbation is:', perturbation.sum() / len(perturbation))
                print('success rate is:', sum(list_success_fail) / len(list_success_fail))
    ...
    if 'MPBA' == item['attack']:
        ...
        torch.save(dict_save,'./result/imagenet-first100/{}_attack_{}_pnorm_{}_GenNum{}_stepsize{}_mu_{}-{}_lower{}_upper{}_c={}.pt'.format(item['model'],item['attack'],item['p_norm'],item['generation_num'],item['stepsize_start'],item['mu_start'],item['mu_end'],item['lower'],item['upper'],item['c']))
    elif 'SquareAttack' in item['attack']:
        torch.save(dict_save,'./result/imagenet-first100/{}_attack_{}_pnorm_{}_epsilon{}_iternum_{}.pt'.format(item['model'], item['attack'], item['p_norm'], item['eps'], item['steps']))
