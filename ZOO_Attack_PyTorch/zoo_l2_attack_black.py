...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True,abort_early=True,
              batch_size=128,max_iter=3000,const=0.01,confidence=0.0,early_stop_iters=100, binary_search_steps=3, #Sun:change max_iter=3000
  ...
  upper_bound=1e10
  lower_bound=0.0
  #out_best_attack=input.clone().detach().cpu().numpy() #sun
  out_best_attack = input[0].clone().detach().cpu().numpy()#sun
...
if __name__=='__main__':
  ...
  #test_set = datasets.CIFAR10(root = '/home/metaphysics/Desktop/my_experiment/datasets/', train=False, transform = transform, download=True)
  ...
  if use_log:
    valid_class = np.argmax(F.softmax(model(torch.from_numpy(inputs).cuda()),-1).detach().cpu().numpy(),-1)  #???? why the input to model is between [-0.5,0.5] instead of [0,1]
